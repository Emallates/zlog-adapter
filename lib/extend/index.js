      ...
      if(!_.isFunction(cb)) throw Error("Callback function is required on total()");
       ...
       waterfall([function(done){
          ...
          qObj.field('*'); qObj.where([{method:"GET"}]); qObj.interval(_interval);
           ...
           if(err) { cb(err, false); return; } 
               var finalObj = []; 
               finalObj = mapAxes(0, "key", "Read", "value", data.results, finalObj);
               done(null, finalObj);
           });
          ...
          qObj.field('*'); qObj.where([{method:"POST"}]); qObj.interval(_interval);
           qObj.exec(function(err, data){
           if(err) { cb(err, false); return; }  
               finalObj = mapAxes(1, "key", "Create", "value", data.results, finalObj);
               done(null, finalObj);
           });
       }, function(finalObj, done){
          qObj.type('fhistory');
          qObj.field('*'); qObj.where([{method:"PUT"}]); qObj.interval(_interval);
           qObj.exec(function(err, data){
           if(err) { cb(err, false); return; }  
               finalObj = mapAxes(2, "key", "Update", "value", data.results, finalObj);
               done(null, finalObj);
           });
       }, function(finalObj, done){
          qObj.type('fhistory');
          qObj.field('*'); qObj.where([{method:"DELETE"}]); qObj.interval(_interval);
           qObj.exec(function(err, data){
           if(err) { cb(err, false); return; }  
               finalObj = mapAxes(3, "key", "Delete", "value", data.results, finalObj);
               done(null, finalObj);
           });
       }],cb);
...
_extend.prototype.systemStatusPerMethod = function(cb){
  try{
      if(!_.isFunction(cb)) throw Error("Callback function is required on total()");
       var qObj = new query(this);
       waterfall([function(done){
          qObj.select('code').type('classify').where({method:"GET"});
          qObj.exec(function(err, data){
               var finalObj = {};
               finalObj.Read = data.facets;
               finalObj.time = data.time;
               done(null, finalObj);
          });    
       }, function(finalObj, done){
          qObj.select('code').type('classify').where({method:"POST"});
          qObj.exec(function(err, data){
               finalObj.Create = data.facets;
               done(null, finalObj);
          });    
       }, function(finalObj, done){
          qObj.select('code').type('classify').where({method:"PUT"});
          qObj.exec(function(err, data){
               finalObj.Update = data.facets;
               done(null, finalObj);
          }); 
       }, function(finalObj, done){
          qObj.select('code').type('classify').where({method:"DELETE"});
          qObj.exec(function(err, data){
               finalObj.Delete = data.facets;
               done(null, finalObj);
          }); 
       }, function(finalObj, done){
            /*START FROM HERE model 
              {
                "key": "Failure",
                "color": "#d62728",
                "values": [{"label" : "Create" , "value" : -10 }] join all values from create, delete, update, read
                predict failure from the codes and join the same result from 200:456,500:456,204:50 501:4
                value ----> 456 +4
              },
              {
                "key": "Success",
                "color": "#57AE57",
                "values": [{"label" : "Create" ,"value" : 20 }] join all values
              } 
            */
            cb(null, finalObj);
