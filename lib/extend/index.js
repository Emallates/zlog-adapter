...
_extend.prototype.rules = function(model, Callback){
   if(ValidRule(model)){ 
     model = parser(model);
     var body = model.ftype == 'set' ? {body:model.rule, url: this.adapter.host+'/engine'+this._add , method : "POST"} : setQS(model,{body:model.rule, url: this.adapter.host+'/engine'+this._get+'/'+this._token , method : "GET"}); 
...
function parser(model){
  ...
  if(model.type == 'operator') { 
     if(model.rule.op.function.type == "Function") model.rule.op.function = JSONEX.stringify(model.rule.op.function); }
  if(model.type == 'keyword') { 
    if(model.rule.kw.function.type == "Function") model.rule.kw.function = JSONEX.stringify(model.rule.kw.function); 
  }
  return model;
}
