...
function cleanBrowser(data){
  data.browsers = data.messages; 
  data.total = data.total_results;
  for(var i in data.browsers){  delete data.browsers[i].timestamp; delete data.browsers[i].ref; }
  delete data.missing; delete data.total_results; delete data.other; delete data.messages; 
  return data
}

function getClassByCode(obj){
  var final_obj = {"ERRORS":0, EXCEPTIONS:0, SUCCESSES:0, timestamp: obj.timestamp};
  for(var i in obj.terms)if(i[0] == "2" || i[0] == "3") final_obj.SUCCESSES+=obj.terms[i];else if(i[0] == "4") final_obj.ERRORS+=obj.terms[i];else final_obj.EXCEPTIONS+=obj.terms[i];
  return final_obj;
}
